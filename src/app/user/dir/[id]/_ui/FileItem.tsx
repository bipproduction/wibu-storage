...
import { MdDelete, MdEdit, MdFileCopy, MdFileDownload, MdFileOpen, MdShare } from "react-icons/md";
...
export function FileItem({
    ...
    file: Prisma.FilesGetPayload<{ select: { id: true; name: true, ext: true, mime: true } }>;
    width: number;
    selectedId: string;
    ...
    const onCopy = () => {
        window.navigator.clipboard.writeText(apis["/api/files/[id]"]({ id: file.id }));
        setContextMenu("");
        ...
        <Stack>
            ...
            <Menu opened={contextMenu === file.id}>
                        ...
                        <Stack gap={0} align={"center"} justify={"end"} >
                            ...
                            <Box bg={selectedId === file.id ? "blue" : "transparent"} style={{
                                ...
                                {selectedId === file.id && isRename ? (
                                    ...
                                    <Text ta={"center"} c={"white"} lineClamp={2} fz={"12"}>{file.name}</Text>
                                )}
                            </Box>
                        </Stack>
                    </Paper>
                ...
                <Menu.Dropdown>
                    ...
                    <Menu.Item onClick={() => onDoubleClick(file.id)} leftSection={<MdFileOpen size={14} />}>Open</Menu.Item>
                    <Menu.Item onClick={() => setIsRename(!isRename)} leftSection={<MdEdit size={14} />}>Rename</Menu.Item>
                    <Menu.Item onClick={onDelete} leftSection={<MdDelete size={14} />}>Delete</Menu.Item>
                    <Menu.Item onClick={onCopy} leftSection={<MdFileCopy size={14} />}>Copy</Menu.Item>
                    <Menu.Item leftSection={<MdShare size={14} />}>Share</Menu.Item>
                    <Menu.Item component={"a"} download={file.name} href={apis["/api/files/[id]"]({ id: file.id })} leftSection={<MdFileDownload size={14} />}>Download</Menu.Item>
                </Menu.Dropdown>
            </Menu>
        </Stack>
    );
